  ...
  <div class="bg-decore-modal mx-auto rounded-[30px] overflow-hidden shadow-lg bg-[#EBE4DF] text-[#5F2C3E]">
    ...
    <div class="relative z-[1] pt-[42px] px-[80px] pb-[20px]">
      ...
      <div class="">
        ...
        <URadioGroup v-model="selectedTime" orientation="horizontal" indicator="hidden" variant="card" :items="times"
          class="flex gap-4 flex-wrap" :ui="{
      ...
      <button @click="addToCart" :disabled="selectedExtension === ''"
        ...
        <span>{{ selectedService && selectedExtension ? (selectedService.price + ' SAR - ') : '' }}Continue</span>
      </button>
    </div>
    <!-- calendar end -->
...
const selectedService = computed(() => menuModule.getSelectedService);
const selectedExtension = computed(() => menuModule.selectedExtension);

// const finalPrice = computed(() => {
//   if (!selectedService.value || !selectedExtension.value) return null;
//   return selectedService.value.price ?? null;
// });


const finalPrice = computed(() => {
  if (!selectedService.value || !selectedExtension.value) return null;
  const extension = selectedService.value.extensionOptions?.find(
    (e: any) => e.value === selectedExtension.value
  );
  return extension?.price ?? selectedService.value.price ?? null;
});
